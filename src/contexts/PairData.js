...
const getHourlyRateData = async (pairAddress, startTime, latestBlock) => {
  try {
    const utcEndTime = dayjs.utc()
    let time = startTime
    const Interval = 3600 || 6057 || Math.max(parseInt((utcEndTime.unix() - time) / 100), 900)
    // create an array of hour start times until we reach current hour
    const timestamps = []
    while (time <= utcEndTime.unix() - Interval) {
      timestamps.push(time)
      time += Interval
    }
    console.log({ Interval, timestamps });
...
export function useHourlyRateData(pairAddress, timeWindow) {
  ...
  useEffect(() => {
    ...
    const windowSize = timeWindow === timeframeOptions.WEEK ? 'week' : 'day'
    const startTime =
      timeWindow === timeframeOptions.ALL_TIME ? 1611022400 : currentTime.subtract(1, windowSize).startOf('hour').unix()

    async function fetch() {
      data = await getHourlyRateData(pairAddress, startTime, latestBlock)
  ...
  useEffect(() => {
    async function fetchData() {
      if (!pairData && pairAddress) {
        ...
        console.log(pairAddress, ethPrice, data);
